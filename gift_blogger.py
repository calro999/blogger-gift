...
2. HTML本文の構成：
   - 記事全体を `<div class="gifuru-gift-article">` と `</div>` で囲む
   - 商品の魅力的な説明（`<div class="gifuru-content-body">` と `</div>` で囲む）
   - おすすめの魅力ポイント3選（`<ul class="gifuru-points-list">` と `<li>` タグを使用）
    ...
    if github_token:
        try:
            ...
            payload = {
                "model": "gpt-4o-mini",
                "messages": [
                    {"role": "system", "content": "あなたはギフト専門セレクトガイド『ギフル』の編集長兼専属ライターです。様々なTPO（誕生日、記念日、お祝い、プチギフトなど）に合わせたセンス抜群のトレンドギフトや、自分を甘やかす至高のご褒美おしゃれスイーツを厳選して紹介します。読者の『何を贈ればいいか分からない』という悩みを解決する、上品で洗練された、かつ魅力的な紹介記事を日本語のみで執筆してください。毎回完全にユニークで、テンプレートの使い回し感のない文章を作成してください。指示された仕様に完全に従い、前置きやHTMLタグブロックのマークダウン表現などを含めない純粋なHTML本文のみを出力します。"},
...
3. ハッシュタグを3〜5個（商品のカテゴリやギフト・スイーツに関連するもの）含め、末尾に「#楽天市場」を必ず含めること。
4. URLや疑似リンク、プレースホルダー（「[リンクはこちら]」など）は絶対に含めないでください。
5. 出力は紹介コメントのテキストのみとし、前置きやMarkdownの装飾コードブロック等は一切含めないでください。
"""

    system_message = "あなたはギフト専門セレクトガイド『ギフル』の専属インフルエンサーです。TPOに合わせたセンス抜群のギフトや、自分を甘やかす至高のご褒美おしゃれスイーツを楽天ROOMのフォロワーに向けて紹介します。思わず『コレ！』したくなるような魅力を伝える紹介文を日本語のみで執筆してください。"
    ...
    return f"【ギフル厳選トレンドギフト】\n\n本当にセンス抜群でおすすめしたい素敵アイテムをご紹介します✨\nご褒美スイーツやお祝いギフトにぴったり🎀\n\n{clean_title}...\n\n#楽天市場 #おすすめギフト #ギフト #ご褒美スイーツ"
